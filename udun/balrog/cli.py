...
def get_nightly_blob_name(productName, branch, build_type, suffix,
                          dummy=False):
    if dummy:
...
_URL = 'http://%s/?product=%%PRODUCT%%&os=%%OS_BOUNCER%%&lang=%%LOCALE%%'
_FTP = '%supdate/%%OS_FTP%%/%%LOCALE%%/%%FILENAME%%'
_FTP2 = "%supdate/%%OS_FTP%%/%%LOCALE%%/%s"
_URL2 = 'http://%s/?product=%s&os=%%OS_BOUNCER%%&lang=%%LOCALE%%'
        ...
        if partialUpdates:
            ...
            for previousVersion, previousInfo in partialUpdates.iteritems():
                from_ = get_release_blob_name(productName, previousVersion,
                                              ...
                                              self.dummy)
                filename = "%s-%s-%s.partial.mar" % (
                    productName.lower(), previousVersion, version)
                bouncerProduct = "%s-%s-partial-%s" % (
                    productName.lower(), version, previousVersion)
        ...
        for channel in uniqueChannels:
            ...
            if "localtest" in channel:
                dir_ = makeCandidatesDir(productName.lower(), version,
                                         buildNumber, server=ftpServer,
                                         protocol='http')
                filename = "%s-%s.complete.mar" % (
                    productName.lower(), version)
                filename = _FTP2 % (dir_, filename)
                data["fileUrls"][channel]["completes"]["*"] = filename
            else:
                # See comment above about these channels for explanation.
                if not requiresMirrors and channel in ("beta", "beta-cdntest"):
                    bouncerProduct = "%s-%sbuild%s-complete" % (
                        productName.lower(), version, buildNumber)
                else:
                    if productName.lower() == "fennec":
                        bouncerProduct = "%s-%s" % (
                            productName.lower(), version)
                    else:
                        bouncerProduct = "%s-%s-complete" % (
                            productName.lower(), version)
                url = _URL2 % (bouncerServer, bouncerProduct)
        ...
        for channel in uniqueChannels:
            ...
            for previousVersion, previousInfo in partialUpdates.iteritems():
                ...
                if "localtest" in channel:
                    ...
                    filename = "%s-%s-%s.partial.mar" % (
                        productName.lower(), previousVersion, version)
                    data["fileUrls"][channel]["partials"][from_] = _FTP2 % (
                        dir_, filename)
                else:
                    lname = productName.lower()

                    # See comment above about these channels for explanation.
                    if (not requiresMirrors and
                            channel in ("beta", "beta-cdntest")):
                        bouncerProduct = "%s-%sbuild%s-partial-%sbuild%s" % (
                            lname, version, buildNumber, previousVersion,
                            previousInfo["buildNumber"])
                    else:
                        bouncerProduct = "%s-%s-partial-%s" % (
                            productName.lower(), version, previousVersion)
...
class NightlySubmitterBase(object):
    ...
    def run(self, platform, buildID, productName, branch, appVersion, locale,
            hashFunction, extVersion, schemaVersion,
            isOSUpdate=None, **updateKwargs):
        assert schemaVersion in (
            3, 4), 'Unhandled schema version %s' % schemaVersion
        ...
        if platform == 'android-api-9':
            ...
            # Like 1055305, this is a hack to support two builds with same
            # build target that
